...
TO_SCRAP_URL = JSON_DATA_URL


def format_new(data: dict):
    image_url = dict.get(dict.get(data["promo_items"], "basic", {}), "url", None)

    return New(
        title=data["headlines"]["basic"],
        description=data["subheadlines"]["basic"],
        created_at=dateparser.parse(data["display_date"]).astimezone(tz_utc),
        image_url=image_url,
        original_url=BASE_URL + data["websites"]["peru21"]["website_url"],
        website=BASE_URL,
    )
...
async def scrap() -> List[New]:
    ...
    data: List[New] = [format_new(element) for element in body["content_elements"]]
